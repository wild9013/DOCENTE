...
# el presente codigo permite al usuario escoger una carpeta inicial 
# y dentro de esa carpeta buscara otras con igual nombre para 
# sincronizar
class SyncApp:
    def __init__(self, root):
        ...
        # Botones para eliminar y sincronizar
    ...
    def remove_folder(self):
        ...
        for item in selected:
            ...
            self.folder_list.delete(item)
            self.log(f"Carpeta eliminada de la lista: {folder}")
        ...
        for rel_path, (latest_path, latest_time, source_folder) in all_files.items():
            for target_folder in self.folders:
                ...
                if not os.path.exists(target_path):
                    ...
                    self.log(f"Desde {source_folder}: Copiado {os.path.basename(rel_path)} -> {target_folder}")
                elif os.path.getmtime(target_path) < latest_time:
                    shutil.copy2(latest_path, target_path)
                    self.log(f"Desde {source_folder}: Actualizado {os.path.basename(rel_path)} -> {target_folder}")
