...
class Juego100Colombianos:
    def __init__(self, root):
        ...
        # Seleccionar archivo Excel mediante ventana gráfica
        archivo = self.seleccionar_archivo()
        if not archivo:  # Si no se selecciona archivo, cerrar la aplicación
            self.root.quit()
            return
        
        # Leer preguntas desde el archivo seleccionado
        self.preguntas = self.leer_preguntas_excel(archivo)
        self.score = 0
    ...
    def seleccionar_archivo(self):
        """Abre una ventana gráfica para seleccionar el archivo Excel."""
        archivo = filedialog.askopenfilename(
            title="Seleccionar archivo Excel",
            filetypes=[("Archivos Excel", "*.xlsx *.xls"), ("Todos los archivos", "*.*")]
        )
        if not archivo:
            messagebox.showwarning("Advertencia", "No se seleccionó ningún archivo. La aplicación se cerrará.")
        return archivo
