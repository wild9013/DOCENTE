...
def parsear_preguntas(archivo):
    ...
    Cada pregunta es un diccionario con:
      ...
      - 'correcta': la letra de la opción correcta (detectada mediante la marca ((x)) o "RESPUESTA:")
    """
    try:
        with open(archivo, 'r', encoding='utf-8') as f:
            contenido = f.read()
    ...
    for conjunto in conjuntos_raw[1:]:
        ...
        for bloque in bloques_preguntas:
            ...
            # Procesar líneas de opciones (se asume que están en lineas[1:-1])
            ...
            # Si no se ha marcado la opción correcta con ((x)), se revisa la última línea
            if pregunta['correcta'] is None and lineas[-1].startswith("RESPUESTA:"):
                respuesta = lineas[-1].split(":", 1)[1].strip()
...
def generar_pdf(conjuntos, header_info, archivo_salida, archivo_fondo, alpha_fondo=0.3):
    ...
    # Dibujar la introducción y las preguntas
