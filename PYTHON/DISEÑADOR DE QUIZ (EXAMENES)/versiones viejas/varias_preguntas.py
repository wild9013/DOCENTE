...
def parsear_preguntas(archivo):
    ...
    # Dividir el archivo en conjuntos de preguntas
    conjuntos_raw = re.split(r'INTRODUCCION:', contenido, flags=re.IGNORECASE)
    conjuntos = []
    
    for conjunto in conjuntos_raw[1:]:  # Omitir la primera parte si está vacía
        ...
        conjuntos.append({'introduccion': introduccion, 'preguntas': preguntas})
    
    return conjuntos

def generar_pdf(conjuntos, archivo_salida):
    """Genera un PDF con múltiples conjuntos de preguntas."""
    ...
    for conjunto in conjuntos:
        ...
        for pregunta in preguntas:
            if y_position < margen_y + 50:
                c.showPage()
                y_position = alto - margen_y
            
            texto_pregunta = f"{pregunta['pregunta']}"
            p = Paragraph(texto_pregunta, estilo_pregunta)
            p.wrap(ancho - 2*margen_x, alto)
            p.drawOn(c, margen_x, y_position - p.height)
            ...
            for letra, texto_opcion in pregunta['opciones'].items():
                ...
                opcion.wrap(ancho - 2*margen_x - 20, alto)
                opcion.drawOn(c, margen_x + 12, y_position - opcion.height)
                y_position -= opcion.height + 3
