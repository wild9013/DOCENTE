...
class ExamAnalysisApp:
    ...
    def analyze_questions(self):
        ...
        results = []
        # Mapeo de opciones numéricas a etiquetas de letras
        option_mapping = {'1': 'Opción A', '2': 'Opción B', '3': 'Opción C', '4': 'Opción D'}
        ...
        for i in range(self.num_questions):
            ...
            # --- Asegura que todas las opciones posibles (Opción A, B, C, D) estén en el contador ---
            mapped_counts = {option_mapping.get(opt, opt): count for opt, count in response_counts.items()}
            for option in ['Opción A', 'Opción B', 'Opción C', 'Opción D']:
                mapped_counts.setdefault(option, 0)

            # --- Calcula el número y porcentaje de respuestas correctas ---
            correct_count = mapped_counts.get(option_mapping.get(correct_answer, correct_answer), 0)
            percentage = (correct_count / total_students * 100) if total_students > 0 else 0

            results.append({
                'number': i + 1,
                'correct': option_mapping.get(correct_answer, correct_answer),  # Usa la etiqueta mapeada
                'correct_count': correct_count,
                'percentage': percentage,
                'distribution': dict(sorted(mapped_counts.items())),
                'options': ['Opción A', 'Opción B', 'Opción C', 'Opción D']  # Usa etiquetas fijas
            })
    ...
    def create_chart(self, question):
        ...
        """
        options = question['options']  # Ya son ['Opción A', 'Opción B', 'Opción C', 'Opción D']
    ...
    def update_interface(self):
        ...
        self.lbl_correct.config(text=f"Respuesta Correcta: {question['correct']}")
        self.lbl_correct_count.config(text=f"Respuestas Correctas: {question['correct_count']} ({question['percentage']:.1f}%)")
        dist_text = " | ".join([f"{k}: {v}" for k, v in question['distribution'].items()])
