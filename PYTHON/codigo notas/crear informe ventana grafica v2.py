...
def generar_informes():
    ...
    ruta_guardado = entry_guardado.get()  # Ruta donde se guardarán los archivos
    ...
    # Crear la carpeta para los informes
    ...
    for nombre, grupo_estudiante in estudiantes_grupo:
        ...
        # Reemplazar marcadores en los párrafos de la plantilla
        ...
        # Reemplazar marcadores dentro de las tablas
        ...
        # Guardar el documento de Word en la carpeta especificada
        documento.save(os.path.join(ruta_guardado, f'informe_{nombre.replace(" ", "_")}.docx'))

    messagebox.showinfo("Éxito", f"Informes generados exitosamente en la carpeta: {ruta_guardado}")
...
# Nueva entrada para la ruta donde se guardarán los informes
tk.Label(ventana, text="Ruta de Guardado:").grid(row=4, column=0, padx=10, pady=10)
entry_guardado = tk.Entry(ventana, width=30)
entry_guardado.grid(row=4, column=1, padx=10, pady=10)
tk.Button(ventana, text="Seleccionar", command=lambda: entry_guardado.insert(0, filedialog.askdirectory(title="Seleccionar Carpeta de Guardado"))).grid(row=4, column=2, padx=10, pady=10)

# Botón para generar informes
tk.Button(ventana, text="Generar Informes", command=generar_informes).grid(row=5, columnspan=3, padx=10, pady=20)
