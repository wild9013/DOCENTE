...
class CurriculumSelectorApp:
    ...
    def setup_ui(self):
        """Initialize and configure the user interface with customtkinter styling."""
        main_frame = ctk.CTkFrame(self.root, padx=25, pady=25)
        ...
        # File Selection
        border_color = ctk.ThemeManager.theme.get("color", {}).get("label_fg", "gray70")
        file_frame = ctk.CTkFrame(top_frame, border_width=2, border_color=border_color)
        file_frame.grid(row=0, column=0, padx=(0, 15), pady=10, sticky="ew")
        ctk.CTkLabel(file_frame, text="Archivo", font=ctk.CTkFont(weight="bold")).pack(padx=10, pady=(5, 0), anchor="nw")
        ctk.CTkButton(file_frame, text="Seleccionar Archivo Excel", command=self.load_file).pack(fill="x", padx=10, pady=(0, 8))

        # Filters
        filter_frame = ctk.CTkFrame(top_frame, border_width=2, border_color=border_color)
        filter_frame.grid(row=0, column=1, padx=(15, 0), pady=10, sticky="ew")
        filter_frame.columnconfigure(1, weight=1)
        ctk.CTkLabel(filter_frame, text="Filtros", font=ctk.CTkFont(weight="bold")).pack(padx=10, pady=(5, 0), anchor="nw")
        ...
        self.search_scroll_y.grid(row=0, column=1, sticky="ns", padx=(0, 10), pady=5)
        ...
        # Results Section
        result_frame = ctk.CTkFrame(main_frame, border_width=2, border_color=border_color)
        result_frame.pack(fill="both", expand=True, pady=(0, 20))
        ctk.CTkLabel(result_frame, text="Resultados", font=ctk.CTkFont(weight="bold")).pack(padx=10, pady=(5, 0), anchor="nw")
    ...
    def resize_search_results(self, event=None):
        """Adjust Listbox size based on content."""
        search_frame_width = max(self.search_results.winfo_width(), 200)
        max_width = 200
        for item in self.search_results_data:
            text = f"Grado {item['grado']} - Período {item['período']} - Eje Temático: {item['eje tematico (numeros)'][:50]}..."
            max_width = max(max_width, self.get_text_width(text))
        listbox_width = min(max_width, int(search_frame_width * 0.9)) // 8
    ...
    def update_periods(self, event):
        """Update period dropdown based on grade."""
        if self.df is not None and self.grade_var.get():
            ...
            self.search_results_data = []
            self.search_data()
            self.status_var.set("Seleccione un período o busque contenido.")
        else:
            self.period_combo.configure(values=[])
            self.period_var.set('')
            self.search_results.delete(0, tk.END)
    ...
    def search_data(self):
        ...
        if search_query:
            ...
            filtered_df = filtered_df[
                filtered_df[search_fields].apply(
        ...
        if not filtered_df.empty:
            for _, row in filtered_df.iterrows():
                display_text = f"Grado {row['grado']} - Período {row['período']} - Eje Temático: {row['eje tematico (numeros)'][:50]}..."
                self.search_results.insert(tk.END, display_text)
                self.search_results_data.append(row.to_dict())
            self.status_var.set(f"{len(filtered_df)} resultados encontrados.")
        else:
    ...
    def on_result_select(self, event):
        """Update result labels based on selected Listbox item."""
